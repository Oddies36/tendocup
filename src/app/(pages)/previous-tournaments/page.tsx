...
export default async function Previoustournaments() {
  ...
  const statusStyles: Record<string, { color: string; label: string }> = {
    setup: { color: "text-gray-500", label: "Création" },
    ongoing: { color: "text-green-500", label: "En cours" },
    finished: { color: "text-red-500", label: "Terminé" },
  };
  ...
  return (
    <main className="pt-20">
      ...
      <div>
        <h1
          className={`${quantico.className} text-[40px] text-center p-10 mb-5`}
        >
          TOURNOIS PRÉCÉDENTS
        </h1>
      </div>
      ...
      <div className="content-center">
        <div className="max-w-6xl mx-auto p-4 grid place-items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {tournaments.map((t) => {
              const current =
                statusStyles[t.status] ?? { color: "text-gray-400", label: t.status };

              return (
                <div key={t.id}>
                  <Link href={`/tournament/${t.id}/${t.status}`}>
                    <div className="text-center cursor-pointer border p-4 rounded-md bg-stone-200 w-60 h-40 transition delay-50 duration-300 ease-in-out hover:scale-110 hover:border-red-400 shadow-md">
                      ...
                      <span className={`mt-3 font-bold ${current.color}`}>
                        {current.label}
                      </span>
                    </div>
                  </Link>
